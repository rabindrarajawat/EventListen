...
const {
  ...
  uniqueDnaTorrance,
  useRootTraitType,
} = require(path.join(basePath, "/config.js"));
...
const buildSetup = (tokenID) => {
  if (!fs.existsSync(buildDir)) {
    // fs.rmdirSync(buildDir, { recursive: true });
    fs.mkdirSync(buildDir);
    fs.mkdirSync(path.join(buildDir, `/json/${tokenID}.json`));
  }
  ...
  // console.log("🚀 ~ file: main.js ~ line 65 ~ buildSetup ~ buildDir", buildDir)
};
...
const outputFiles = (abstractedIndexes, layerData, metadataList, attributesList, tokenID) => {
  ...
  const metadata = addMetadata(newDna, tokenID, {
    _prefix,
    _offset,
  }, attributesList);
  ...
  saveMetaDataSingleFile(tokenID, metadata);
  console.log(
    chalk.cyan(
      `Created edition: ${tokenID}, with DNA: ${hash(newDna)}`
    )
  );
};

const startCreating = async (storedDNA, layerConfigurations, tokenID) => {
  console.log("🚀 ~ file: main.js ~ line 771 ~ startCreating ~ storedDNA", storedDNA)
  console.log("🚀 ~ file: main.js ~ line 770 ~ startCreating ~ layerConfigurations", JSON.stringify(layerConfigurations))
  if (storedDNA) {
  ...
  for (let i = startIndex; i <= startIndex + layerConfigurations[layerConfigurations.length - 1].growEditionSizeTo - 1; i++) {
    abstractedIndexes.push(i);
    console.log("🚀 ~ file: main.js ~ line 786 ~ startCreating ~ abstractedIndexes.push(i)", abstractedIndexes.push(i));
  }
  if (shuffleLayerConfigurations) {
    console.log("🚀 ~ file: main.js ~ line 789 ~ startCreating ~ shuffleLayerConfigurations", shuffleLayerConfigurations)
    abstractedIndexes = shuffle(abstractedIndexes);
    console.log("🚀 ~ file: main.js ~ line 791 ~ startCreating ~ abstractedIndexes", abstractedIndexes)
  }
  debugLogs
    ? console.log("Editions left to create: ", abstractedIndexes)
    : null;
  while (layerConfigIndex < layerConfigurations.length) {
    ...
    console.log("🚀 ~ file: main.js ~ line 800 ~ startCreating ~ layers", layers)
    while (
      editionCount <= layerConfigurations[layerConfigIndex].growEditionSizeTo
    ) {
      let newDna = createDna(layers);
      console.log("🚀 ~ file: main.js ~ line 805 ~ startCreating ~ newDna", newDna)
      ...
      // console.log("🚀 ~ file: main.js ~ line 814 ~ allImages ~ allImages", allImages)
      ...
      await Promise.all(loadedElements).then((renderObjectArray) => {
        ...
        console.log("🚀 ~ file: main.js ~ line 821 ~ awaitPromise.all ~ layerData", layerData)
        paintLayers(ctxMain, renderObjectArray, layerData, attributesList);
        outputFiles(abstractedIndexes, layerData, metadataList, attributesList, tokenID);
      });
